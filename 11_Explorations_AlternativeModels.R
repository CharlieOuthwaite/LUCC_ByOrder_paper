...
############  1. Abundance model - no rescaling ############ 
...
############  2. Including order as a random effect ############ 

# Here I will try including insect order as a random effect as well as a 
# fixed effect. This aims to address additional variation between orders that
# is not picked up by the site or study random effects. Some studies assess 
# multiple orders. 


# directories
datadir <- "5_RunLUIClimateModels/"
outdir <- "10_Additional_Tests/Order_random_effect/"
if(!dir.exists(outdir)) dir.create(outdir)
...
MeanAnomalyModelAbund <- GLMER(modelData = model_data,responseVar = "LogAbund",fitFamily = "gaussian",
                               fixedStruct = "LUI * StdTmeanAnomalyRS * Order",
                               randomStruct = "(1|SS)+(1|SSB)+(1|Order)",
                               saveVars = c("SSBS"))
...
# save the model output
save(MeanAnomalyModelAbund, file = paste0(outdir, "MeanAnomalyModelAbund_orderRanEff.rdata"))
...
MeanAnomalyModelRich <- GLMER(modelData = model_data,responseVar = "Species_richness",fitFamily = "poisson",
                              fixedStruct = "LUI * StdTmeanAnomalyRS * Order",
                              randomStruct = "(1|SS)+(1|SSB)+(1|SSBS)+(1|Order)",
                              saveVars = c("SSBS"))

summary(MeanAnomalyModelRich$model)

save(MeanAnomalyModelRich, file = paste0(outdir, "MeanAnomalyModelRich_orderRanEff.rdata"))
