...
# load packages
packages_plot <- c("patchwork","dplyr","ggplot2","scatterpie","gt","broom.mixed","MASS","webshot","sjPlot","stringr","rjson","rgdal","maptools","sf", "broom")
...
p_map1.1 <-ggplot() +
  geom_polygon(data = shp_df, aes(x = long, y = lat, group = group, fill = id)) +
  coord_fixed() +
  #labs(fill = "Order") +
...
p_map1.1 <-ggplot() +
  ...
  geom_polygon(data = shp_df, aes(x = long, y = lat, group = group), alpha = 0.5, col = "black", fill = c("#E0EEEE")) +
 # scale_fill_paletteer_d("ggsci::default_igv") +
  ...
  #geom_jitter() + 
  scale_colour_manual(values=c("#0F6B99FF","#FFD8B2FF","#A3CC51FF","#8F7EE5FF","#990F0FFF"))+
  theme(axis.title.x = element_blank(),
        axis.title.y = element_blank(),
        axis.text = element_blank(),
        axis.ticks = element_blank(),
...
#save map (mercator)
ggsave("map_sites_mercator_UN.jpeg", device ="jpeg", path = plotDir, width=25, height=15, units="cm", dpi = 350)
